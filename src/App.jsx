...
// ─────────────────────────────────────────────
function LeadForm({contacts,setContacts,usuarios}) {
  const [form,setForm]=useState({nombre:"",email:"",telefono:"",empresa:"",servicio:"marketing",notas:""});
  ...
  const submit=async()=>{
    ...
    setSaving(true);
    const initials=form.nombre.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase();
    const vendores=usuarios.filter(u=>u.rol==="vendedor");
    const vendedor=vendores[contacts.length%vendores.length];
    const data=await sb.post("contactos",{...form,avatar:initials,valor:SERVICES.find(s=>s.id===form.servicio)?.precio||15000,vendedor_id:vendedor?.id,interacciones:0,etapa:"Nuevo Lead",fecha:new Date().toISOString().slice(0,10)});
    if(data&&data[0])setContacts(prev=>[...prev,data[0]]);
    setSent(true);setSaving(false);
    setTimeout(()=>{setSent(false);setForm({nombre:"",email:"",telefono:"",empresa:"",servicio:"marketing",notas:""});},3000);
  };

  return(
    <div style={{display:"flex",flexDirection:"column",gap:20}}>
      <div>
        <h3 style={{margin:0,fontSize:16,fontWeight:800}}>📥 Formulario de Registro de Leads</h3>
        <p style={{margin:"4px 0 0",fontSize:12,color:"#9ca3af"}}>Simula la llegada de un lead desde tu web · Se guarda en Supabase y se asigna automáticamente</p>
      </div>
      {sent?(
        <div style={{background:"#dcfce7",border:"1px solid #86efac",borderRadius:12,padding:40,textAlign:"center"}}>
          <div style={{fontSize:40,marginBottom:10}}>✅</div>
          <div style={{fontWeight:800,fontSize:16,color:"#166534"}}>¡Lead registrado en Supabase!</div>
          <div style={{fontSize:13,color:"#166534",marginTop:6}}>Asignado automáticamente a un asesor</div>
        </div>
      ):(
        <Card>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
              {[["Nombre completo *","nombre","text"],["Email *","email","email"],["Teléfono","telefono","tel"],["Empresa","empresa","text"]].map(([label,field,type])=>(
                <div key={field} style={{flex:1,minWidth:170}}>
                  <label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b7280",marginBottom:4,textTransform:"uppercase",letterSpacing:.5}}>{label}</label>
                  <input type={type} value={form[field]} onChange={e=>setForm(f=>({...f,[field]:e.target.value}))} style={{width:"100%",padding:"10px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:13,boxSizing:"border-box"}}/>
                </div>
              ))}
            </div>
            <div>
              <label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b7280",marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Servicio de interés *</label>
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                {SERVICES.map(s=>(
                  <button key={s.id} onClick={()=>setForm(f=>({...f,servicio:s.id}))}
                    style={{padding:"8px 12px",borderRadius:8,fontSize:12,fontWeight:700,cursor:"pointer",border:"2px solid",background:form.servicio===s.id?s.color:"#f9fafb",color:form.servicio===s.id?"#fff":"#374151",borderColor:form.servicio===s.id?s.color:"#e5e7eb",transition:"all .15s"}}>
                    {s.icon} {s.name}
                  </button>
                ))}
              </div>
            ...
            <div>
              <label style={{display:"block",fontSize:11,fontWeight:700,color:"#6b7280",marginBottom:4,textTransform:"uppercase",letterSpacing:.5}}>Mensaje</label>
              <textarea value={form.notas} onChange={e=>setForm(f=>({...f,notas:e.target.value}))} rows={3} placeholder="¿En qué podemos ayudarte?" style={{width:"100%",padding:"10px 12px",border:"1px solid #e5e7eb",borderRadius:8,fontSize:13,resize:"none",boxSizing:"border-box"}}/>
            </div>
            <button onClick={submit} disabled={saving} style={{padding:"13px",b
